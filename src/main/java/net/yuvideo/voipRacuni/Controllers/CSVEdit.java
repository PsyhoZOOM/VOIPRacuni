...
public class CSVEdit implements Initializable {
  ...
  private void initInterface() {
    ...
    tblCSVDAta.getColumns()
        .addAll(cCustomerID, cAccount, cFrom, cTo, cCountry, cDescription, cConnecTime,
            cChargedAmountRSD, cChargedTimeMS);
    ...
    bIzbrisi.setOnAction(new EventHandler<javafx.event.ActionEvent>() {
      ...
      public void handle(javafx.event.ActionEvent event) {
        ...
        Alert aler = new Alert(Alert.AlertType.CONFIRMATION,
            "Da li ste sigurni da želite da izbrišete?", yes, no);
        aler.setTitle("Brisanje CSV-a");
        aler.showAndWait();
        if (aler.getResult() == no) {
          return;
        }
        ObservableList<CSVData> csvDataArrayList = tblCSVDAta.getSelectionModel()
            .getSelectedItems();
