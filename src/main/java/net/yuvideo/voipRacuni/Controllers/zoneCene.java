...
public class zoneCene implements Initializable {
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    cProviderCenaPDV
        .setCellValueFactory(new PropertyValueFactory<ZoneCene, Double>("providerUkupno"));
  ...
  public void showNov(ActionEvent actionEvent) {
    FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/FXML/editUsluge.fxml"),
        resources);
    ...
    try {
      ...
      if (rs.isBeforeFirst()) {
        while (rs.next()) {
          ...
          zoneCene.setProviderUkupno(rs.getDouble("providerCena")
              + rs.getDouble("providerCena") * rs.getDouble("providerPDV") / 100);
          zoneCene.setCena(rs.getDouble("cena"));
          zoneCene.setPDV(rs.getDouble("PDV"));
          zoneCene
              .setCenaPDV(rs.getDouble("cena") + rs.getDouble("cena") * rs.getDouble("PDV") / 100);
