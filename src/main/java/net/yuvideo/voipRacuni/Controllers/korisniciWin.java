...
public class korisniciWin implements Initializable {
  ...
  private void showUplate() {
    FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/FXML/KorisnikUplate.fxml"),
        resources);
    try {
      ...
      stage.setTitle(
          "Uplate korisnika: " + tblKorisnici.getSelectionModel().getSelectedItem().getIme());
  ...
  public void novKorisnik(ActionEvent actionEvent) {
    FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/FXML/editKorisnik.fxml"),
        resources);
    try {
      Scene scene = new Scene(fxmlLoader.load());
  ...
  public void editKorisnik(ActionEvent actionEvent) {
    ...
    try {
      ...
      stage.initModality(Modality.APPLICATION_MODAL);
      stage.setTitle(
          "Izmena korisnika " + tblKorisnici.getSelectionModel().getSelectedItem().getIme() + " "
              + tblKorisnici.getSelectionModel().getSelectedItem().getCustomerId());
      stage.setScene(scene);
      stage.showAndWait();
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public void obrisiKorinisk(ActionEvent actionEvent) {
    if (tblKorisnici.getSelectionModel().getSelectedIndex() == -1) {
      Alert alert = new Alert(Alert.AlertType.ERROR, "Nije izabran ni jedan korisnik za brisanje",
          ButtonType.OK);
      alert.showAndWait();
      return;
    }
    ...
    Alert alert = new Alert(Alert.AlertType.CONFIRMATION,
        String.format("Da li ste sigurni da želite da izbrišete" +
            "korisnika %"));
