...
public class statistika implements Initializable {
  ...
  public void showStatistika(ActionEvent actionEvent) {
    ...
    ArrayList<CSVData> allCSVData = FIXX
        .getSumCountryChargetTimeS(db, dtpOdDatuma.getValue().toString(),
            dtpDoDatuma.getValue().toString());
    for (CSVData data : allCSVData) {
      pieDoZemlje.getData().add(
          new PieChart.Data(data.getCountry() + " " + data.getChargedTimeSec(),
              data.getChargedTimeSec()));
    }
      ...
      data.getNode().addEventHandler(MouseEvent.MOUSE_PRESSED,
          new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent e) {
              ...
              caption.setText(data.getPieValue() + "min.");
            }
          });
    }
