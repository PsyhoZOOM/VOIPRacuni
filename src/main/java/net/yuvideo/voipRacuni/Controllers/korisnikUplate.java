...
import net.yuvideo.voipRacuni.classes.uplate;
...
public class korisnikUplate implements Initializable {
  ...
  public TreeTableColumn<uplate, String> ctUplaceno;
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    ctUplaceno.setCellValueFactory(new TreeItemPropertyValueFactory<uplate, String>("uplaceno"));
    ctDatumUplate
        .setCellValueFactory(new TreeItemPropertyValueFactory<uplate, String>("datumUplate"));
    ctVrsta.setCellValueFactory(new TreeItemPropertyValueFactory<uplate, String>("komentar"));

    ctImePrezime.setCellFactory(
    ...
    tblUplateTree.getSelectionModel().selectedItemProperty()
        .addListener(new ChangeListener<TreeItem<uplate>>() {
          @Override
  ...
  public void setData() {
    ...
    for (TreeItem<uplate> treupl : root.getChildren()) {
      ...
      if (treupl.getValue().getUplaceno().equals("Ne") && treupl.getValue().getKomentar()
          .equals("Paket")) {
    ...
    try {
      ...
      if (rs.isBeforeFirst()) {
        while (rs.next()) {
          ...
          if (rs.getDouble("uplaceno") > 0) {
            uplata.setUplaceno("Da");
          } else {
            uplata.setUplaceno("Ne");
          }
