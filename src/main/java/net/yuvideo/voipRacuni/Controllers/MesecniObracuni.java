...
public class MesecniObracuni implements Initializable {
  ...
  public void check_if_obracun_exists(LocalDate value) {
    ...
    if (exist) {
      bObracunaj.setDisable(true);
      lMessage.setText(String
          .format("Obracun za mesec %s postoji", dtpObracunZaMesec.getValue().format(dateFormat)));
  ...
  public void obracunaj(ActionEvent actionEvent) {
    ...
    obracunZaMesec = LocalDate.of(obracunZaMesec.getYear(), obracunZaMesec.getMonthValue(),
        obracunZaMesec.lengthOfMonth());

    zaduziKorisnike zaduziKorisnike = new zaduziKorisnike(users, obracunZaMesec.toString(), db);

    bObracunaj.setDisable(true);

    lMessage.setText(String.format("Obracun za mesec %s  je završen. Možete zatvoriti ovaj prozor.",
        dtpObracunZaMesec.getValue().format(dateFormat)));
