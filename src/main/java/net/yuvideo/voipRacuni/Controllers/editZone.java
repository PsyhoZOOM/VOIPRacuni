...
public class editZone implements Initializable {
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    tblZone.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Zone>() {
      @Override
      public void changed(ObservableValue<? extends Zone> observable, Zone oldValue,
          Zone newValue) {
        if (newValue == null) {
          return;
        }
  ...
  public void izmeniZonu(ActionEvent actionEvent) {
    if (cmbZona.getSelectionModel().isEmpty()
        || tblZone.getSelectionModel().getSelectedIndex() == -1) {
      return;
    }
    PreparedStatement ps;
