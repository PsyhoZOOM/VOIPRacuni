...
public class FIXX {
  ...
  public static ArrayList<CSVData> getAllCSVData(Database db, String startDate, String stopDate) {
  ...
  public static ArrayList<CSVData> getSumCountryChargetTimeS(Database db, String StartDate,
      String StopDate) {
    PreparedStatement ps;
    ResultSet rs;
    String query = "SELECT sum(chargedTimeS) as chargedTimeS, country FROM csv WHERE connectTime >= ? AND connectTIme <= ? GROUP BY country";
    ArrayList<CSVData> csvDataArrayList = new ArrayList<>();
    try {
      ps = db.connection.prepareStatement(query);
      ps.setString(1, StartDate);
      ...
      if (rs.isBeforeFirst()) {
        while (rs.next()) {
          ...
          csvData.setChargedTimeSec(rs.getInt("chargedTimeS") / 60);
          csvData.setCountry(rs.getString("country"));
          csvDataArrayList.add(csvData);
        }
      }
  ...
  public static ArrayList<CSVData> getCSVDataDescriptionDistinct(int userID, String accountPhone,
      String startDate, String endDate, Database db) {
    ArrayList<CSVData> csvDataArrayList = new ArrayList<>();
    PreparedStatement ps;
    ResultSet rs;
    //chargedTimeS > 0 zbog nekih nebuloza u csv fajlu
    String query =
        "SELECT distinct(description) FROM csv WHERE account=? AND connectTime >= ? AND connectTime <= ? "
            +
            "and chargedTimeS != 0  ";
    try {
      ps = db.connection.prepareStatement(query);
  ...
  public int getCSVDataChargedTimeS_SUM(String account, String description, String startDate,
      String stopDate, Database db) {
    PreparedStatement ps;
    ResultSet rs;
    // chargedTimeS > 0 FIX zbog nekih nebuloza u CSV fajlu
    String query =
        "SELECT sum(chargedTimeS) AS seconds FROM csv WHERE account=? and description=? and " +
